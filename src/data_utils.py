...
def get_data(ds_name='pg19', ds_dir='datasets', batch_size=16):
	...
	dataloaders={split:DataLoader(data[split], batch_size=batch_size) for split in splits}

	return data, dataloaders

def tokenize_data(data, ds_name='pg19', save_dir=None):
	"""
	...
	if os.path.exists(save_dir):
		tokenized_data = datasets.load_from_disk(save_dir)
...
def build_vocab(data, ds_name='pg19', token_ds_dir=None, vocab_dir=None, min_freq=3):
	"""
	...
	# build vocab
	print('building vocabulary (this only needs to happen once per dataset)...')
